...
class ReDo(BaseReDo):
    ...
    def step(self, obs: torch.Tensor) -> Dict[str, any]:
        ...
        # Check if in reset range (修复：单个整数判断)
        in_reset_range = True
        if self.reset_steps is not None:
            in_reset_range = self.current_step >= self.reset_steps
...
class GradientReDo(BaseReDo):
    ...
    def __init__(self, model: nn.Module, mode: str = 'threshold', tau: float = 0,
                 ...
                 optimizer: optim.Adam = None, reset_steps: int = None):
    ...
    def _get_layer_mask(self, layer: nn.Module, tau: float) -> torch.Tensor:
        ...
        if self.mode == 'threshold':
            ...
            threshold = torch.kthvalue(grad_magnitude, k).values if k < len(grad_magnitude) else torch.min(
                grad_magnitude)
